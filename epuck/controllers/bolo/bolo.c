...
static bool cliff_detected() {
  ...
  for (i = 0; i < DISTANCE_SENSORS_NUMBER; i++) {
    if (!distance_sensors[i])
      return false;
    if (distance_sensors_values[i] < 0.0138)
      return true;
  }
  return false;
}
...
static void run_braitenberg(bool rodar) {
  if (rodar){
    int i, j;
    for (i = 0; i < 2; i++) {
      speeds[i] = 0.0;
...
int main(int argc, char **argv) {
  ...
  wb_robot_init();
  bool rodar = true;
  double* cini = malloc(8*sizeof(double));
  double* cfin = malloc(8*sizeof(double));
  for(int i = 0; i < 8; i++){
    char c[6];
    snprintf(c,sizeof(double)+5,"caixa%d",i+1);
    WbNodeRef boxNode = wb_supervisor_node_get_from_def(c);
    WbFieldRef positionField = wb_supervisor_node_get_field(boxNode, "translation");
    const double* c_pos = wb_supervisor_field_get_sf_vec3f(positionField);
    cini[i] = c_pos[0];
  }
  init_devices();
  int i = 0;
  while (rodar) {
    for(int j = 0; j < 8; j++){
    char c[6];
    snprintf(c,sizeof(double)+5,"caixa%d",j+1);
    WbNodeRef boxNode = wb_supervisor_node_get_from_def(c);
    WbFieldRef positionField = wb_supervisor_node_get_field(boxNode, "translation");
    const double* c_pos = wb_supervisor_field_get_sf_vec3f(positionField);
    cfin[j] = c_pos[0];
    ...
    get_sensor_input();
    for(int j = 0; j < 8; j++){
      if(arredonda(cini[j]) != arredonda(cfin[j])){
        set_actuators();
        wb_motor_set_velocity(left_motor, 0.0);
        wb_motor_set_velocity(right_motor, 0.0);
        blink_leds();
